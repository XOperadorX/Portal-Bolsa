...
(function() {
    ...
    class LevelSystem {
        ...
        async verificarAdmin() {
            ...
            try {
                ...
                if (data && data.cadeado === true) {
                    ...
                    btnAdmin.style.display = 'none';
                    btnAdmin.classList.remove('visivel');
                    console.log('🔒 Admin bloqueado (cadeado = false)');
                }
            } catch (e) {
            ...
            try {
                if (this.usuario && this.supabaseOnline) {
                    ...
                    if (error) {
                        ...
                        this.mostrarToast('⚠️ Erro ao salvar stamina, mas continuando...', 'aviso');
                    } else {
                        console.log(`✅ SM salvo no Supabase: ${this.sm}`);
                    }
                } else {
                    console.warn('⚠️ Supabase offline ou usuário não logado — SM não persistido.');
                }
            } catch (e) {
        ...
        async criarRegistroNivel(usuario) {
            try {
                ...
                if (errSelect) {
                    console.error('❌ Erro ao verificar existência:', errSelect);
                    return false;
                }
        ...
        async loadUserData() {
            try {
                ...
                // 👑 VERIFICA ADMIN APÓS CARREGAR USUÁRIO
                await this.verificarAdmin();
    ...
    console.log('👑 Verificação de admin por cadeado ativa!');
})();